...
# ---------------- UI HEADER ----------------
st.title("🧠 Brain Tumor Segmentation AI")
st.markdown("Upload MRI scan and get segmentation with clinical-style visualization")
...
def preprocess(image):
    ...
    # dynamic size from model (fallback 256)
    try:
        _, _, H, W = input_shape
        H = H if isinstance(H, int) else 256
        W = W if isinstance(W, int) else 256
...
if file:
    ...
    with col1:
        st.subheader("Original MRI")
        orig_display = np.array(image.resize((256, 256)))
        st.image(orig_display, use_container_width=True)

    # PREPROCESS
    inp, (H, W) = preprocess(image)
    ...
    try:
        ...
        # ---------------- CIRCLE ----------------
        circle = get_tumor_circle(mask_resized)
        if circle:
        ...
        with col3:
            st.subheader("Clinical Annotation")
        ...
        with c3:
            st.metric("Model Output", "Segmentation OK")

        # ---------------- EXPLANATION ----------------
        st.info(
            "Green contour shows exact tumor boundary. "
            "Blue circle highlights overall tumor region detected by AI segmentation model."
        )
